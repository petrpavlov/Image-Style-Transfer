...
def vgg_19(inputs,
    ...
    with tf.variable_scope(scope, 'vgg_19', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
            ...
            net = slim.repeat(net, 4, slim.conv2d, 256, [3, 3], scope='conv3', trainable=False)
            net = slim.avg_pool2d(net, [2, 2], scope='pool3')
            net = slim.repeat(net, 4, slim.conv2d, 512, [3, 3], scope='conv4', trainable=False)
            net = slim.avg_pool2d(net, [2, 2], scope='pool4')
            net = slim.repeat(net, 4, slim.conv2d, 512, [3, 3], scope='conv5', trainable=False)
            ...
            if num_classes:
                net = slim.dropout(net, dropout_keep_prob, is_training=is_training,
                                   scope='dropout7', trainable=False)
                net = slim.conv2d(net, num_classes, [1, 1],
                                  activation_fn=None,
                                  normalizer_fn=None,
                                  scope='fc8')
